...
class MathGame:
    def __init__(self, master):
        ...
        master.configure(borderwidth=10, relief='ridge', bg="#F9C7BE")  # Border and background color
        master.geometry("800x500")  # Window size
    ...
    def save_name(self):
        ...
        if re.match(r'^[a-zA-Z\s]+$', name):  # Check if name contains only letters and spaces
            filename = "saved_progress.txt"
            with open(filename, "a") as file:
                file.write(name + "\n")  # Save the name to a file
            self.error_window("Saved!")  # Show a confirmation message
        else:
    ...
    def run_easy_level(self):
        # Create the easy level screen
        self.easy_level_frame = Frame(self.master, bg="#F9C7BE")
        self.easy_level_frame.pack(fill=BOTH, expand=True)

        # Add a label for the question
        self.question_label = Label(self.easy_level_frame, text="", font=(self.kfont, 16), bg="#F9C7BE")
        self.question_label.pack(pady=20)

        # Add an entry box for the user to enter their answer
        self.answer_entry = Entry(self.easy_level_frame, font=(self.kfont, 12))
        self.answer_entry.pack(pady=10)

        # Add a label for feedback (correct/incorrect)
        self.feedback_label = Label(self.easy_level_frame, text="", font=(self.kfont, 16), bg="#F9C7BE")
        self.feedback_label.pack(pady=10)

        # Add a button to submit the answer and go to the next question
        next_button = Button(self.easy_level_frame, text="Next", command=self.check_answer, bg="#CE6A6C", height=2, width=10, fg="white")
        next_button.pack(pady=20)

        # Generate the first easy question
        self.easy_question()
    ...
    def check_answer(self):
        ...
        if int(user_answer) == self.correct_answer:
            ...
            # Display the correct answer if the user is incorrect
    ...
    # Pause for 1s before going to the next question
        self.master.after(1000, self.next_easy_question)


    def next_easy_question(self):
        ...
        if self.current_question < self.total_questions:
            ...
            self.easy_question()  # Generate the next question
        else:
    ...
    def show_results(self):
        ...
        result_message = f"You got {self.correct_answers} out of {self.total_questions} correct!"
        messagebox.showinfo("Results", result_message)
        self.easy_level_frame.pack_forget()  # Hide the easy level screen
