...
class MathGame:
    ...
    def load_gif(self):
        try:
            openImage = Image.open(self.gif_path)
            frames = openImage.n_frames
            self.imageObject = [PhotoImage(file=self.gif_path, format=f"gif -index {i}") for i in range(frames)]
    ...
    def update_gif(self):
        if hasattr(self, 'gif_label') and self.gif_label and self.imageObject:
            new_image = self.imageObject[self.current_frame]
            self.gif_label.configure(image=new_image)
            self.current_frame = (self.current_frame + 1) % len(self.imageObject)
    ...
    def character_selection(self):
        ...
        # Adjust the GIF label to fit with the grid layout
        self.gif_label = Label(self.character_selection_frame)
        self.gif_label.grid(row=0, column=1, rowspan=3, padx=20, pady=20)
