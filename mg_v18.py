...
class MathGame:
    ...
    def display_waddle_happy(self):
        self.happy_label = Label(self.level_frame, image=self.waddle_happy_image, bg=self.level_frame.cget("bg"))
    ...
    def run_level(self, question_method, level):
        ...
        """
        # Background colour depending on which character you selected
        if self.selected_character == "Waddle Dee": bg_color = "#fab175" 
        else: bg_color = "#ebcacf"
    
        self.level_frame = Frame(self.master, bg=bg_color)
    ...
    def check_answer(self):
        ...
        if user_answer_int == self.correct_answer:
            ...
            self.display_waddle_happy()  # Display the happy image
        else:
