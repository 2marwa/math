'''This is a math quiz game to imrpove and test math skills '''
...
class MathGame:
    ...
    def update_gif(self):
        ' ' ' Update the GIF images regularly.' ' '
    ...
    def character_selection(self):
        ...
        # Configure grid row and column weights for proper expansion
        self.character_selection_frame.grid_rowconfigure(6, weight=1)
        self.character_selection_frame.grid_columnconfigure(0, weight=1)
        self.character_selection_frame.grid_columnconfigure(10, weight=1)

        # Exit button (bottom right)
        Button(self.character_selection_frame, text="Exit", command=self.close_window, bg="#fefcbf", height=2, width=10, fg="black").grid(row=6, column=10, pady=10, padx=10, sticky="se")
        
        # Instructions button 
        Button(self.character_selection_frame, text="Instructions", command=lambda: messagebox.showinfo(
            "Instructions", 
            "How to complete the Star Sum Quest:\n\n"
            "1. Choose your character.\n"
            "2. Answer math questions to collect stars.\n"
            "3. Progress through levels of increasing difficulty.\n"
            "4. Try to achieve the highest score!\n\n\n"
            "Progress will be saved once entire game is complete, so don't leave halfway!\n\n"
            "Tip: Round your answers to the nearest whole number"
        ), bg="#fefcbf", height=2, width=10, fg="black").grid(row=6, column=0, pady=10, padx=10, sticky="sw")
    ...
    def save_name(self):
        ' ' ' Validate and save the player name. ' ' '
        ...
        if re.match(r'^[a-zA-Z\s]+$', name):  # Validation
            ...
            messagebox.showerror("Error", "Letters a-z only!")
            return  
    ...
    def run_level(self, question_method, level):
        ...
        if self.selected_character == "Waddle Dee":
            ...
            BG_COLOUR = "#fcd1ae"  # Label background color for Waddle Dee
        else:
            BG_IMAGE_PATH = os.path.join(os.path.dirname(__file__), "k_bg.jpg")
            BG_COLOUR = "#ebcacf"  # Label background color for Kirby
        ...
        self.score_label = Label(self.level_frame, text=f"Score: {self.score}", font=(self.KFONT, 12), bg=BG_COLOUR)
        self.score_label.pack(anchor=NE, padx=10, pady=5)

        # Display question label
        self.question_label = Label(self.level_frame, text="", font=(self.KFONT, 16), bg=BG_COLOUR)
        ...
        # Feedback label
        self.feedback_label = Label(self.level_frame, text="", font=(self.KFONT, 16), bg=BG_COLOUR)
    ...
    def level4_question(self):
        ...
        question_text = f"What is {n1} ÷ {n2} (rounded)?"
        self.question_label.config(text=question_text)
    ...
    def show_level_complete(self):
        ...
        BG_COLOUR = "#e8b58b" if self.selected_character == "Waddle Dee" else "#d6abb2"
        self.level_complete_frame = Frame(self.master, bg=BG_COLOUR)
        self.level_complete_frame.pack(fill=BOTH, expand=True)

        # Display level completion message
        level_complete_label = Label(self.level_complete_frame, text=f"\n\nYou've completed\n{self.level}!", font=(self.KFONT, 24), bg=BG_COLOUR)
    ...
    def next_level(self):
        ...
        if hasattr(self, 'level_complete_frame') and self.level_complete_frame.winfo_ismapped():
            self.level_complete_frame.pack_forget()  # Hide the level completion screen
    ...
    def show_game_complete(self):
        '''Show a screen when the game is completed.'''

        if hasattr(self, 'level_complete_frame') and self.level_complete_frame.winfo_ismapped():
            self.level_complete_frame.pack_forget()  # Hide the level completion screen

        BG_COLOUR = "#e8b58b" if self.selected_character == "Waddle Dee" else "#d6abb2"
        self.game_complete_frame = Frame(self.master, bg=BG_COLOUR)
        self.game_complete_frame.pack(fill=BOTH, expand=True)

        # Display game completion message
        game_complete_label = Label(self.game_complete_frame, text="\n\nCongratulations!\nYou've completed the game!", font=(self.KFONT, 24), bg=BG_COLOUR)
        game_complete_label.pack(pady=20)

        # Exit button
        exit_button = Button(self.game_complete_frame, text="Exit", command=self.close_window, bg="#fefcbf", height=2, width=10, fg="black")
        exit_button.pack(side=BOTTOM, anchor=SE, padx=10, pady=10)
